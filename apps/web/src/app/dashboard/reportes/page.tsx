    ...
    <div className="space-y-4">
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all capitalize ${
              activeBook === b
                ...
                : 'text-[var(--cx-text-secondary)] hover:text-slate-300'
            }`}
          >
      ...
      <div className="card border border-[var(--cx-border-light)] rounded-2xl overflow-hidden">
        ...
        <div className="divide-y divide-[var(--cx-border-light)]">
          {LCV_VENTAS.map(r => (
            <div key={r.folio} className="grid grid-cols-12 gap-2 px-4 py-3 text-sm hover:bg-[var(--cx-hover-bg)] transition-colors">
              <div className="col-span-1 text-[var(--cx-text-primary)] font-mono">#{r.folio}</div>
              <div className="col-span-1 text-[var(--cx-text-secondary)]">{r.tipo}</div>
              <div className="col-span-3 text-[var(--cx-text-primary)] truncate">{r.receptor}</div>
              <div className="col-span-2 text-[var(--cx-text-secondary)] font-mono text-xs">{r.rut}</div>
              <div className="col-span-1 text-[var(--cx-text-secondary)] text-xs">{r.fecha.slice(5)}</div>
              <div className="col-span-2 text-right text-slate-300">{formatCLP(r.neto)}</div>
              <div className="col-span-1 text-right text-[var(--cx-text-secondary)] text-xs">{formatCLP(r.iva)}</div>
              <div className="col-span-1 text-right text-[var(--cx-text-primary)] font-semibold">{formatCLP(r.total)}</div>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-12 gap-2 px-4 py-3 border-t border-[var(--cx-border-light)] bg-[var(--cx-bg-elevated)] text-sm font-bold">
          ...
          <div className="col-span-2 text-right text-[var(--cx-text-primary)]">{formatCLP(totalNeto)}</div>
          <div className="col-span-1 text-right text-[var(--cx-text-secondary)]">{formatCLP(totalIva)}</div>
