/**
 * CUENTAX — Nueva Solicitud de Compra
 * Formulario para crear un nuevo pedido de compra.
...
export default function NuevaSolicitudCompraPage() {
  ...
  const defaultEntrega = new Date(Date.now() + 7 * 86400000).toISOString().split('T')[0]
  ...
  // Validation
  const isValid = rutProveedor.length >= 9 && razonSocial.length >= 2 && items.every(i => i.nombre.length > 0 && i.cantidad > 0)
  ...
  async function handleSave(enviar: boolean) {
    ...
    try {
      const payload = {
        ...
        email_proveedor: emailProveedor || undefined,
        items: items.map(i => ({
          nombre: i.nombre,
          cantidad: i.cantidad,
      ...
      const { data: created } = await apiClient.post('/api/v1/compras/pedidos', payload)

      // If enviar, also send to supplier
      if (enviar && created?.id) {
        await apiClient.post(`/api/v1/compras/pedidos/${created.id}/enviar`)
      }
  ...
  return (
    <div className="space-y-6 animate-fade-in max-w-4xl">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-[var(--cx-text-primary)]">Nueva Solicitud de Compra</h1>
          <p className="text-sm text-[var(--cx-text-secondary)] mt-0.5">Crea un pedido de compra para tu proveedor</p>
        </div>
      </div>

      {/* Supplier Info */}
      <div className="card p-5 space-y-4">
        <h2 className="text-sm font-semibold text-[var(--cx-text-primary)] uppercase tracking-wide">Datos del Proveedor</h2>
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-xs text-[var(--cx-text-muted)] mb-1">RUT Proveedor *</label>
            <input
              type="text"
              value={rutProveedor}
          ...
          <div>
            ...
            <input
              ...
              placeholder="Proveedor ABC Ltda."
              className="input-field w-full"
            />
          </div>
          <div>
            <label className="block text-xs text-[var(--cx-text-muted)] mb-1">Email Proveedor</label>
            <input
              type="email"
          ...
          <div>
            <label className="block text-xs text-[var(--cx-text-muted)] mb-1">Fecha de Entrega</label>
            <input
              type="date"
              value={fechaEntrega}
