 ...
 * Mia: "Esta es la pantalla más usada del producto.
...
function ModoSimple({ onSwitch }: { onSwitch: () => void }) {
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center gap-4 py-12 animate-fade-in">
        ...
        <h3 className="text-lg font-bold text-white">¡DTE Emitido!</h3>
        <p className="text-sm text-slate-400">Folio #1043 · Enviado al SII</p>
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div>
        <label className="block text-xs font-medium text-slate-400 mb-3">¿Qué quieres emitir?</label>
            ...
            <button
              ...
              className={`
                ...
                ${tipo === t.value
                  ...
                  : 'bg-white/[0.02] border-white/[0.06] text-slate-400 hover:border-white/10 hover:text-slate-300'
                }
              `}
            >
      ...
      {/* Preview monto */}
      {monto && (
        <div className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.06] animate-fade-in">
          ...
          <div className="flex justify-between text-sm font-semibold text-white mt-2 pt-2 border-t border-white/[0.06]">
            <span>Total</span>
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 animate-fade-in">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-medium text-slate-400 mb-1.5">Tipo DTE</label>
      ...
      <div className="p-4 rounded-xl bg-white/[0.02] border border-white/[0.06] space-y-3">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Receptor</p>
        <div className="grid grid-cols-2 gap-3">
          <div>
          ...
          <div>
            <label className="block text-xs text-slate-500 mb-1">Giro</label>
      ...
      <div>
        ...
        <div className="grid grid-cols-12 gap-2 px-2 pb-1">
          {['Descripción', 'Cant.', 'Precio', 'Desc%', ''].map((h, i) => (
            <span key={i} className={`text-[10px] text-slate-600 uppercase tracking-wide ${
        ...
        <div className="space-y-2">
          {fields.map((field, idx) => {
            ...
            return (
              <div key={field.id} className="grid grid-cols-12 gap-2 items-center p-3 rounded-xl bg-white/[0.02] border border-white/[0.04]">
                ...
                <div className="col-span-1 text-right">
                  <span className="text-xs text-slate-400">{formatCLP(monto)}</span>
                </div>
                <button
                  type="button"
                  onClick={() => fields.length > 1 && remove(idx)}
                  className="col-span-1 flex justify-center text-slate-700 hover:text-red-400 transition-colors disabled:opacity-30"
      ...
      {/* Totales */}
      <div className="p-4 rounded-xl bg-slate-900 border border-white/[0.07] space-y-2">
        <div className="flex justify-between text-xs text-slate-500">
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-white">Emitir DTE</h1>
        ...
        <button
          ...
          className={`
            ...
            ${mode === 'expert'
              ? 'bg-violet-500/10 border-violet-500/30 text-violet-300'
              : 'bg-white/[0.03] border-white/[0.07] text-slate-400 hover:text-slate-300'
            }
          `}
        >
